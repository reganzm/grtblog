    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable) // 防止跨站请求伪造
                ...
                .authorizeHttpRequests(authorizeRequests -> authorizeRequests
                        .requestMatchers("/user/login", "/user/logout", "/test1").permitAll() // 登录和未登录的人都可以访问
        ...
        // 自定义登出成功处理器
        http.logout(logout -> logout.logoutUrl("/logout").logoutSuccessHandler(new LogoutStatusSuccessHandler()));
        return http.build();
    }
}
